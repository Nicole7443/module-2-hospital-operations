...
wb = openpyxl.load_workbook(base_dir + "MSE 433 Hospital Operations Excel Tool.xlsm", keep_vba=True)
sheet = wb['Dept Breakdown'] #Access the specific sheet by name

#Extract Inputs from specific cells
#ER Department
...
#Penalty applied to patients REMAINING in waiting room (not transferred)
quality_penalty = (
    pen_additional_staff_q * gp.quicksum(a[i] for i in depts) + pen_sharing_rooms * gp.quicksum(s[i] for i in sharing_depts) +
...
for i in depts:
    ...
    #Conservation: Transferred patients must be included in department counts
    #Total low severity patients in dept = existing + transferred
    if i in sharing_depts:
        model.addConstr(l[i] + s[i] + rl_i[i] >= rl_i[i])  # Ensures transferred are counted
    else:
        model.addConstr(l[i] + rl_i[i] >= rl_i[i])
    
    #High severity patients in dept = existing + transferred
    model.addConstr(h[i] + rh_i[i] >= rh_i[i])
...
z_score = model.objVal  
cost_score = cost_penalty.getValue()
quality_score = quality_penalty.getValue()  
...
for i in depts:
    ...
    ws.cell(row=row, column=6, value=int(s[i].X) if i in sharing_depts else None)  
